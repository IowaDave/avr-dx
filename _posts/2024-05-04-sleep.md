...
One technique is to ```sleep``` whenever the device is waiting any length of time for the next task to begin. When a microcontroller spends most of its time waiting, considerable power reduction can be achieved by spending it in a sleep mode rather than leaving the device in active mode.
